...
def create_identity(entity, name, identity_type, creator, **kwargs):
    ...
    if not existing_identity:
        identity = models.Identity(name=name, identifier=name, identity_type=identity_type, is_private=False, is_primary=True, is_active=True, created_by=creator)
        identity.save()
    else:
    ...
    if not existing_entity_identity:
        entity_identity = models.EntityIdentity(entity=entity, identity=identity, is_private=False, start_date=timezone.now(), created_by=creator)
...
class Migration(migrations.Migration):
    ...
    def create_federations(app, schema_editor):
        ...
        global_federation = dict(name='World Athletics', aliases=['International Amateur Athletic Federation', 'International Association of Athletics Federations', 'IAAF'], formation=timezone(1912, 7, 17), website='www.worldathletics.org')
        ...
        federations = [
            dict(name='Asian Athletics Association', aliases=['Asian Athletics Association', 'AAA'], formation=timezone(1912, 7, 17), website='www.athleticsasia.org'),
            dict(name='Confederation of African Athletics', aliases=['Confederation of African Athletics', 'CAA'], formation=timezone(1912, 7, 17), website='www.caaweb.org'),
            dict(name=u'Confederación Sudamericana de Atletismo', aliases=[u'Confederación Sudamericana de Atletismo', 'CONSUDATLE'], formation=timezone(1918, 1, 1), website='www.consudatle.org'),
            dict(name='European Athletic Association', aliases=['European Athletic Association', 'European Athletics', 'EAA'], formation=timezone(1969, 1, 1), website='www.european-athletics.org'),
            dict(name='North American, Central American and Caribbean Athletic Association', aliases=['North American, Central American and Caribbean Athletic Association', 'NACAC'], formation=timezone(1988, 1, 1), website='www.athleticsnacac.org'),
            dict(name='Oceania Athletics Association', aliases=['Oceania Athletics Association', 'OAA'], formation=timezone(1969, 8, 21), website='www.athletics-oceania.com'),
        ]
        ...
        for row in data:
            ...
            if raw_founded_date and raw_founded_date.count('/') == 2:
                founded_date = timezone.strptime(raw_founded_date, '%m/%d/%Y')
            elif raw_founded_date and not raw_founded_date.count('/'):
                founded_date = timezone.strptime(raw_founded_date, '%Y')
            else:
                founded_date = None
            ...
            if raw_affiliation_date and raw_affiliation_date.count('/') == 2:
                affiliation_date = timezone.strptime(raw_affiliation_date, '%m/%d/%Y')
            elif raw_affiliation_date and not raw_affiliation_date.count('/'):
                affiliation_date = timezone.strptime(raw_affiliation_date, '%Y')
            else:
